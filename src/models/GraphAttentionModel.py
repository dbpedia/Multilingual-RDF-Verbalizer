...
class TransGAT(tf.keras.Model):
  ...
  def __init__(self, args, src_vocab_size, src_lang,
    ...
    if args.src_emb != None:
        embedding_weight = np.load(args.src_emb)
        embedding_init = np.zeros(shape=(1,embedding_weight.shape[1]))
        embedding_weight = np.append(embedding_init, embedding_weight, axis=0)
        self.emb_layer = EmbeddingLayer.EmbeddingSharedWeights(
          src_vocab_size, args.emb_dim, fixed=args.fixed_src_emb, embedding_init=embedding_weight)        
    else:
        self.emb_layer = EmbeddingLayer.EmbeddingSharedWeights(
          src_vocab_size, args.emb_dim, fixed=args.fixed_src_emb)

    self.tgt_emb_layer = EmbeddingLayer.EmbeddingSharedWeights(
      tgt_vocab_size, args.emb_dim, fixed=args.fixed_tgt_emb)
